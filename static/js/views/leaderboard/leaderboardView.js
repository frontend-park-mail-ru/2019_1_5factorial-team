...
export default class leaderboardView extends View {
    ...
    onCheckAuthResponse (data = {}) {
        const rightBlockNoAuth = document.querySelector('.main-right-block');
        const rightBlockAuth = document.querySelector('.main-right-block-auth');
        console.log('onCheckAuthResponse data ', data);
    
        if (!data.isAuth) {
            console.log('no auth in menu');
        } else {
            rightBlockNoAuth.classList.add('hidden_users');
            rightBlockAuth.classList.remove('hidden_users');
    ...
    _onLoadUserResponse(data = {}) {
        ...
        console.log('data is ', {userName: data.user.login});
        super.render(null, {userName: data.user.login});

    }

    render(root, data = {}) {
        super.render(root, data);
        this.localEventBus.callEvent('checkAuthorization', {userName: data.login});
